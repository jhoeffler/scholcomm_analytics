...
* Remove data about journals after[^4] they have turned to full open access in observation period using data from the Directory of Open Access Journals
...
[^4]: Total number of articles 142,382 of which are 79,020 hybrid. For journals and publishers need to check due to overlap. The numbers should be made reproducible once the release number of the dataset version is determined.
...
A finer grained approach to see at which step of the data information the numbers of journals by the publishers is reduced in our data will help to determine the influence of delayed access licenses and journal transformations to full open access. As was seen with manual checks, data quality issues also play a role as in some cases ISSNs are not stored correctly and journals change names and publishers.[^6] 
The number of different licenses used is steadily growing. Methods were already developed to harmonize license data and we can further contribute to improve on these approaches[^7]. While the continued introduction of new licenses will remain a challenge, we could show that even for some major publishers this number so far has stayed manageably low.
[^6]: This will be difficult to show reproducibly. I would suggest a link to a list of examples.
[^7]: The agrepl function can help to identify similar licenses. https://stackoverflow.com/questions/45510001/find-similar-strings-and-reconcile-them-within-one-dataframe